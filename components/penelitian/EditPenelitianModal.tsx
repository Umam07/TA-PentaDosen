...
import { X, Pencil, User, Info, Users, Building2, GraduationCap, FileText, ChevronDown, Layers, History, CheckCircle2, Plus } from 'lucide-react';
...
interface EditPenelitianModalProps {
  ...
  onClose: () => void;
  onSave: (updatedItem: ResearchItem) => void;
  onPreview?: (name: string, type: 'pdf' | 'docx', file?: File) => void;
}

export const EditPenelitianModal: React.FC<EditPenelitianModalProps> = ({ isOpen, initialData, onClose, onSave, onPreview }) => {
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      const isStandardSource = initialData.sourceOfFunds === 'Universitas YARSI';
      setFormTitle(initialData.title);
      setFormScheme(initialData.scheme);
      setFormSumberDana(isStandardSource ? 'Universitas YARSI' : 'Lainnya');
      setFormOtherSumberDana(isStandardSource ? '' : (initialData.sourceOfFunds || ''));
      setBudgetProposed(initialData.proposedAmount?.replace(/\D/g, '') || '');
      setBudgetFunded(initialData.amount?.replace(/\D/g, '') || '');
  ...
  }, [isOpen, initialData, setGlobalBlur]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updated: Partial<ResearchItem> = {
      ...
      year: formYear,
      proposalFileName: proposalFile ? generateResearchFileName(formTitle, nidn, proposalFile, 'proposal') : initialData!.proposalFileName,
      proposalFile: proposalFile || initialData!.proposalFile,
      progressReport: progressFile ? generateResearchFileName(formTitle, nidn, progressFile, 'progress') : initialData!.progressReport,
      progressFile: progressFile || initialData!.progressFile,
      finalReport: finalFile ? generateResearchFileName(formTitle, nidn, finalFile, 'final') : initialData!.finalReport,
      finalFile: finalFile || initialData!.finalFile
    };
    
    const updatedItem = { ...updated, status: calculateStatus(updated) } as ResearchItem;
    onSave(updatedItem);
  };
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center">
      ...
      >
        <div className="px-8 py-6 md:px-12 md:py-8 flex justify-between items-center border-b border-slate-50 dark:border-white/5">
          <div className="flex items-center gap-4"><div className="w-12 h-12 bg-primary-600 text-white rounded-[1.25rem] flex items-center justify-center shadow-xl shadow-primary-500/20"><Pencil className="w-6 h-6" /></div><div><h3 className="text-[20px] md:text-[24px] font-black text-slate-900 dark:text-white tracking-tight">Edit Penelitian</h3><p className="text-[12px] text-slate-400 mt-1">Ubah data proposal atau proyek riset Anda.</p></div></div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-8 md:px-12 py-10 custom-scrollbar flex flex-col">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tahun <span className="text-red-500">*</span></label>
                    <div className="relative">
                      <select value={formYear} onChange={(e) => setFormYear(e.target.value)} className="w-full h-[52px] appearance-none px-5 bg-slate-50 dark:bg-neutral-800/50 border border-transparent rounded-2xl text-[14px] font-normal dark:text-white outline-none">
                ...
                <div className="space-y-10 pt-4">
                  <div className="space-y-4">
                    ...
                    <DragAndDropUpload 
                      file={proposalFile} 
                      existingFileName={initialData.proposalFileName}
                      onFileSelect={setProposalFile} 
                      acceptedTypes={['.pdf']} 
                      helperText="Hanya PDF (Max 10MB)" 
                      onPreview={() => onPreview?.(initialData!.proposalFileName || '', 'pdf', proposalFile || initialData!.proposalFile)}
                    />
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 mb-2">
                    ...
                    <DragAndDropUpload 
                      file={progressFile} 
                      existingFileName={initialData.progressReport}
                      onFileSelect={setProgressFile} 
                      acceptedTypes={['.pdf']} 
                      helperText="Hanya PDF (Max 10MB)" 
                      onPreview={() => onPreview?.(initialData!.progressReport || '', 'pdf', progressFile || initialData!.progressFile)}
                    />
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 mb-2">
                    ...
                    <DragAndDropUpload 
                      file={finalFile} 
                      existingFileName={initialData.finalReport}
                      onFileSelect={setFinalFile} 
                      acceptedTypes={['.pdf']} 
                      helperText="Hanya PDF (Max 10MB)" 
                      onPreview={() => onPreview?.(initialData!.finalReport || '', 'pdf', finalFile || initialData!.finalFile)}
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="mt-12 -mx-8 md:-mx-12 px-8 py-8 md:px-12 border-t border-slate-50 dark:border-white/5 flex gap-4 shrink-0"><button type="button" onClick={onClose} className="flex-1 h-[56px] bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-neutral-400 font-bold rounded-2xl active:scale-[0.98]">Batal</button><motion.button {...btnScaleDown} type="submit" className="flex-[2] h-[56px] bg-primary-600 text-white font-black rounded-2xl shadow-xl active:scale-[0.98]">Simpan Perubahan</motion.button></div>
        </form>
      </motion.div>
    </div>
  );
