...
import { X, Info, Mail, Building2, GraduationCap, Phone, User as UserIcon, Camera, Hash } from 'lucide-react';
...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose }) => {
  const { showToast, profilePhoto, setProfilePhoto } = useLayout();
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const avatarUrl = URL.createObjectURL(file) + "?t=" + Date.now();
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="relative group">
                <div className="w-20 h-20 sm:w-24 sm:h-24 bg-primary-600 rounded-[2rem] flex items-center justify-center text-2xl sm:text-3xl font-black text-white shadow-xl shadow-primary-500/20 mb-4 overflow-hidden">
                  {profilePhoto ? (
                ...
                <button 
                  ...
                  className="absolute bottom-2 -right-2 w-8 h-8 bg-white dark:bg-neutral-800 rounded-full shadow-lg border border-slate-100 dark:border-white/10 flex items-center justify-center text-slate-600 dark:text-neutral-300 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
                  title="Ubah Foto"
                >
            ...
            <div className="flex items-start gap-3 p-4 bg-primary-500/[0.04] rounded-2xl border border-primary-500/10">
              ...
              <p className="text-[11px] sm:text-[12px] font-semibold text-primary-800/80 dark:text-primary-400/80 leading-relaxed">
                Data profil dikelola oleh admin dan tidak dapat diubah. Jika terdapat kesalahan data, silakan hubungi admin.
              </p>
            </div>
