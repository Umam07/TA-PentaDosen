...
import { Plus, X, Users, Building2, ChevronDown, ShieldCheck, MapPin, Calendar as CalendarIcon, Layers } from 'lucide-react';
...
export const AddHKIModal: React.FC<AddHKIModalProps> = ({
  isOpen,
  onClose,
  onSave,
  showToast
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const finalStatus = hkiFile ? 'Lengkap' : 'Sedang Berjalan';
    ...
    const itemData: HKIItem = {
      ...
      hkiFileName: hkiFile ? generateFileName(formTitle, nidn, hkiFile) : undefined,
      ...
      >
        <div className="px-8 py-6 md:px-12 md:py-8 flex justify-between items-center border-b border-slate-50 dark:border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-primary-600 text-white rounded-[1.25rem] flex items-center justify-center shadow-xl"><Plus className="w-6 h-6" /></div>
            <div><h3 className="text-[20px] md:text-[24px] font-black text-slate-900 dark:text-white tracking-tight">Tambah HKI Baru</h3><p className="text-[12px] text-slate-400 mt-1">Daftarkan kekayaan intelektual atau paten akademik Anda.</p></div>
          </div>
