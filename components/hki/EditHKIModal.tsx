...
import { Pencil, X, Users, Building2, ChevronDown, ShieldCheck, MapPin, Calendar as CalendarIcon, Layers, Plus } from 'lucide-react';
import { HKIItem, JenisCiptaan, JENIS_OPTIONS, PEOPLE_SUGGESTIONS, backdropVariants, modalVariants, modalTransition, btnScaleDown, generateFileName } from './shared';
import { DragAndDropUpload } from './DragAndDropUpload';

interface EditHKIModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: HKIItem) => void;
  showToast: (msg: string, type: string) => void;
  initialData: HKIItem | null;
}

export const EditHKIModal: React.FC<EditHKIModalProps> = ({
  isOpen,
  onClose,
  onSave,
  showToast,
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setFormJenis(initialData.jenisCiptaan);
      setFormNomorPermohonan(initialData.nomorPermohonan);
      setFormTempatDiumumkan(initialData.tempatDiumumkan);
      setFormTanggalDiumumkan(initialData.tanggalDiumumkan);
      setFormNomorPencatatan(initialData.nomorPencatatan);
      setSelectedPencipta(initialData.pencipta);
      setSelectedPemegang(initialData.pemegang);
      setHkiFile(null);
    }
  }, [isOpen, initialData]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (penciptaRef.current && !penciptaRef.current.contains(event.target as Node)) setIsPenciptaDropdownOpen(false);
      if (pemegangRef.current && !pemegangRef.current.contains(event.target as Node)) setIsPemegangDropdownOpen(false);
  ...
  const citySuggestions = useMemo(() => {
    ...
    return allCities.filter(city => city.toLowerCase().includes(formTempatDiumumkan.toLowerCase())).slice(0, 8);
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formTitle || !formJenis || !formTanggalDiumumkan || !initialData) {
      showToast('Mohon lengkapi field wajib (*)', 'error');
      return;
    }

    const nidn = "1-402022-048";
    const hasAnyFile = hkiFile || initialData?.hkiFileName;
    const finalStatus = hasAnyFile ? 'Lengkap' : 'Sedang Berjalan';
    ...
    const itemData: HKIItem = {
      ...
      hkiFile: hkiFile || initialData.hkiFile,
      hkiFileName: hkiFile ? generateFileName(formTitle, nidn, hkiFile) : initialData.hkiFileName,
      ...
      >
        <div className="px-8 py-6 md:px-12 md:py-8 flex justify-between items-center border-b border-slate-50 dark:border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-primary-600 text-white rounded-[1.25rem] flex items-center justify-center shadow-xl"><Pencil className="w-6 h-6" /></div>
            <div><h3 className="text-[20px] md:text-[24px] font-black text-slate-900 dark:text-white tracking-tight">Edit HKI</h3><p className="text-[12px] text-slate-400 mt-1">Perbarui kekayaan intelektual atau paten akademik Anda.</p></div>
          </div>
          <button onClick={onClose} className="p-3 bg-slate-100 dark:bg-white/5 rounded-2xl text-slate-400 hover:text-slate-900"><X className="w-6 h-6" /></button>
        </div>
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-8 md:px-12 py-10 custom-scrollbar space-y-12">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="space-y-2 col-span-full">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Judul HKI <span className="text-red-500">*</span></label>
               ...
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Jenis Ciptaan <span className="text-red-500">*</span></label>
               ...
               <div className="space-y-2">
                 ...
                 <div className="relative" ref={cityDropdownRef}>
                   ...
                   <input 
                     ...
                     onChange={e => { setFormTempatDiumumkan(e.target.value); setIsCityDropdownOpen(true); setFocusedCityIndex(-1); }} 
                     ...
                     <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl overflow-hidden z-[100]">
                       <div className="max-h-60 overflow-y-auto custom-scrollbar">
                         {citySuggestions.map((city, idx) => (
                           <button
                             key={city} type="button" onClick={() => { setFormTempatDiumumkan(city); setIsCityDropdownOpen(false); }}
                             className={`w-full text-left px-5 py-3 text-[13px] font-medium transition-colors flex items-center gap-3 ${focusedCityIndex === idx ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600' : 'text-slate-700 dark:text-neutral-300 hover:bg-slate-50 dark:hover:bg-white/5'}`}
                           >
                             <MapPin className="w-4 h-4 opacity-50" /> {city}
                           </button>
                         ))}
                       </div>
                     </div>
               ...
               <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tanggal Diumumkan Pertama Kali <span className="text-red-500">*</span></label>
                 <div className="relative">
                    ...
                    <div className="max-h-60 overflow-y-auto p-2 space-y-1">
                      {PEOPLE_SUGGESTIONS.filter(p => p.toLowerCase().includes(penciptaSearch.toLowerCase()) && !selectedPencipta.includes(p)).map(p => (
                        <button key={p} type="button" onClick={() => { setSelectedPencipta([...selectedPencipta, p]); setPenciptaSearch(''); setIsPenciptaDropdownOpen(false); }} className="w-full text-left px-4 py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-white/5 transition-colors flex items-center justify-between group">
                          <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-400 flex items-center justify-center font-black text-[10px]">{p[0]}</div><span className="text-[13px] font-normal text-slate-700 dark:text-neutral-200 group-hover:bg-slate-50/10 transition-colors">{p}</span></div>
          ...
          <div className="space-y-6">
            ...
            <DragAndDropUpload 
              file={hkiFile} 
              existingFileName={initialData?.hkiFileName}
          ...
          <div className="pt-8 flex gap-4">
            <button type="button" onClick={onClose} className="flex-1 h-[56px] border border-slate-200 dark:border-white/10 text-slate-500 font-bold rounded-2xl hover:bg-slate-50 transition-all active:scale-[0.98]">Batal</button>
            <motion.button 
              {...btnScaleDown}
              type="submit" 
              className="flex-[2] h-[56px] bg-primary-600 text-white font-black rounded-2xl shadow-xl shadow-primary-600/20 active:scale-[0.98] transition-all hover:bg-primary-700"
            >
              Simpan Perubahan
            </motion.button>
          </div>
