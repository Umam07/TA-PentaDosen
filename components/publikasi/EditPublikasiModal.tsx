...
import { Pencil, X, User, Info, Users, Building2, GraduationCap, FileText, ChevronDown, Globe, Hash, Layers, Plus } from 'lucide-react';
...
export const EditPublikasiModal: React.FC<EditPublikasiModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialData
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setFormPublisher(initialData.publisher);
      setFormPages(initialData.pages);
      setFormIsbn(initialData.isbn);
      setFormAbstract(initialData.abstract || '');
      setFormYear(initialData.year);
      setSelectedAuthors(initialData.coAuthors);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const hasAnyFile = manuscriptFile || initialData.manuscriptFileName;
    const finalStatus = hasAnyFile ? 'Lengkap' : 'Sedang Berjalan';

    const updatedItem: PublicationItem = {
      ...initialData,
      title: formTitle,
      ...
      manuscriptFile: manuscriptFile || initialData.manuscriptFile,
      manuscriptFileName: manuscriptFile ? generateFileName(formTitle, nidn, manuscriptFile) : initialData.manuscriptFileName
    };
    onSave(updatedItem);
  };
      ...
      >
        <div className="px-8 py-6 md:px-12 md:py-8 flex justify-between items-center border-b border-slate-50 dark:border-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[20px] md:text-[24px] font-black text-slate-900 dark:text-white tracking-tight">Edit Publikasi</h3>
              <p className="text-[12px] text-slate-400 mt-1">Perbarui karya ilmiah atau buku akademik Anda.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-8 md:px-12 py-10 custom-scrollbar space-y-12">
          ...
          <div className="space-y-12">
            <div className="space-y-6">
              ...
              <DragAndDropUpload 
                file={manuscriptFile} 
                existingFileName={initialData.manuscriptFileName}
