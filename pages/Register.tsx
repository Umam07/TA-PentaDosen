...
import { BookOpen, User, Lock, Mail, ChevronLeft, ChevronRight, ArrowRight, Eye, EyeOff, Building2, ChevronDown, CheckCircle2 } from 'lucide-react';
...
// Custom Dropdown Component dengan Animasi Lama
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isStep2Valid()) {
      ...
      setTimeout(() => {
        setIsLoading(false);
        setIsSuccess(true);
        // Automatic redirect to Login after delay
        setTimeout(() => {
          onLogin();
        }, 2000);
      }, 1500);
    ...
    <section className="min-h-screen pt-24 pb-20 flex items-center justify-center px-4 bg-white dark:bg-black transition-colors duration-500 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl">
        ...
        <motion.div 
          ...
          className="bg-white dark:bg-neutral-900/40 border border-slate-100 dark:border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl backdrop-blur-xl relative min-h-[500px] flex flex-col justify-center"
        >
          <AnimatePresence mode="wait">
            {!isSuccess ? (
              ...
              >
                {/* Progress Bar Container dengan Animasi Spring */}
                <div className="mb-10">
                    ...
                    {step === 1 ? (
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          <input name="nidn" value={formData.nidn} onChange={handleInputChange} placeholder="X-XXXXXX-XXX" className={`w-full px-5 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none border transition-all text-slate-900 dark:text-white ${errors.nidn ? 'border-red-500/50' : 'border-transparent focus:border-primary-600/30'}`} />
                        ...
                        <div className="space-y-2">
                          ...
                          <input name="nip" value={formData.nip} onChange={handleInputChange} placeholder="XXXXXXXX-XXXXXX-XX-XX" className={`w-full px-5 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none border transition-all text-slate-900 dark:text-white ${errors.nip ? 'border-red-500/50' : 'border-transparent focus:border-primary-600/30'}`} />
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <div className="relative group">
                            ...
                            <input name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="nama@email.com" className={`w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none border transition-all text-slate-900 dark:text-white ${errors.email ? 'border-red-500/50' : 'border-transparent focus:border-primary-600/30'}`} />
                          </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative group">
                            <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-primary-600 transition-colors" />
                            <input name="username" value={formData.username} onChange={handleInputChange} placeholder="tanpa spasi" className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none focus:border-primary-600/30 border border-transparent transition-all text-slate-900 dark:text-white" />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <div className="relative group">
                              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-primary-600 transition-colors">
                              ...
                              <input 
                                ...
                                onChange={handleInputChange} 
                                placeholder="••••••••" 
                                className="w-full pl-11 pr-12 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none focus:border-primary-600/30 border border-transparent transition-all text-slate-900 dark:text-white" 
                              />
                              <button 
                                type="button" 
                                onClick={() => setShowPassword(!showPassword)} 
                                className="absolute inset-y-0 right-4 flex items-center text-slate-400 hover:text-primary-600 transition-colors"
                              >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative group">
                              ...
                              <input 
                                ...
                                onChange={handleInputChange} 
                                placeholder="••••••••" 
                                className={`w-full pl-11 pr-12 py-3.5 bg-slate-50 dark:bg-neutral-800/50 rounded-2xl text-sm font-medium outline-none border transition-all text-slate-900 dark:text-white ${errors.confirmPassword ? 'border-red-500/50' : 'border-transparent focus:border-primary-600/30'}`} 
                              />
                              <button 
                                type="button" 
                    ...
                    >
                      {isLoading ? (
                        ...
                        <>
                          ...
                          >
                            {step === 1 ? <ChevronRight className="w-4 h-4" /> : <ArrowRight className="w-4 h-4" />}
                          </motion.div>
                        </>
                      )}
                    </button>
              ...
              >
                ...
                <p className="text-slate-500 dark:text-neutral-400 text-base max-w-sm leading-relaxed">
                  Akun Anda berhasil dibuat. Anda akan diarahkan ke halaman masuk dalam beberapa saat.
                </p>
